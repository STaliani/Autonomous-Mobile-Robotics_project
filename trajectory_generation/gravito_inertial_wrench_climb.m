function UGI = gravito_inertial_wrench_climb(surfaces, CoM)
...
%   wrench of the stance
    ...
    for plane=(1:planes)
        ...
        [z1, y1] = meshgrid(MIN_MAX_z(1,plane):0.1:MIN_MAX_z(2,plane), MIN_MAX_y(1,plane):0.1:MIN_MAX_y(2,plane)); % Generate x and y data
        x1=(-planes_coef(plane,3)*z1-planes_coef(plane,2)*y1-planes_coef(plane,4))/planes_coef(plane,1); % Solve for z data
        ...
        tx=[(-planes_coef(plane,3))/planes_coef(plane,1);1;0];
        ty=[(-planes_coef(plane,2))/planes_coef(plane,1);0;1];
        t=tx+ty;
        ...
        for i=(1:np)
            ...
            alpha= 0;
%             if planes_coef(plane,1)*planes_coef(plane,4)<0
%                 beta = -pi/2;
%             else 
%                 beta = pi/2;
%             end
            beta = atan2(planes_coef(plane,1),planes_coef(plane,3));
            Rx = [        1,           0,                0 ;
