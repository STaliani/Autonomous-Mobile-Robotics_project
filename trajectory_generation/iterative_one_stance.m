...
for j=(1:n_stance)
    ...
    opti.minimize(sum(ddp,2)'*sum(ddp,2));
    
    cc = 0;
    
    opti.subject_to(w(1:3, 1) == m*(g-ddp(:,1)));
    opti.subject_to(w(4:6, 1) == cross(p(:,1),m*(g-ddp(:,1))));
    opti.subject_to(U_stance'*w(:,1) <= 0)
    ...
    for r_value = (2:pp_stance)
        ...
        opti.subject_to(dp(:,r_value) >= ones(3,1)*eps)
         
        opti.subject_to(U_stance'*w(:,r_value) <= 0)
    end
    ...
    elseif j==3
        opti.subject_to(x(1:3,1) == [0;6;5])
        opti.subject_to(x(1:3,N) == [0;9;5])
    else
        opti.subject_to(x(1:3,1) == [goal(:,j-1)])
        opti.subject_to(x(1:3,N) == [goal(:,j)])
    end
